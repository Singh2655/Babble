  ...
  return (
    <DropdownMenu>
        ...
        <DropdownMenuContent className="w-56 text-xs font-medium text-neutral-800 hover:text-neutral-400 space-y-[2px]">
            ...
            {isModerator && (
                <DropdownMenuItem className='px-3 py-2 cursor-pointer text-sm' onClick={()=>onOpen("createChannel")}>
                    Create Channel
            ...
            {isAdmin && (
                <DropdownMenuItem className='text-rose-500 px-3 py-2 cursor-pointer text-sm' onClick={()=>onOpen("deleteServer",{server})}>
                    Delete Server
                    <Trash className='h-4 w-4 ml-auto'/>
                    </DropdownMenuItem>
            )}
            {!isAdmin && (
                <DropdownMenuItem className='text-rose-500 px-3 py-2 cursor-pointer text-sm' onClick={()=>onOpen("leaveServer",{server})}>
                    Leave Server
